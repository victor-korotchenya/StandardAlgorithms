...
namespace Standard::Algorithms::Trees
{
    ...
    struct segment_tree_assign_sum final
    {
        static constexpr auto default_size_log = 18U;
        static constexpr auto max_log = 30U;

        constexpr explicit segment_tree_assign_sum(std::uint32_t size_log = default_size_log)
...
private:
        ...
        constexpr void push_down(std::size_t left)
        {
            ...
            do
            {
                ...
                if (auto &has = Debt[parent].second; has)
                {
                    const auto &debt1 = Debt[parent].first;
        ...
        constexpr void push_up(std::size_t parent)
        {
            ...
            for (;;)
            {
                ...
                const auto new_value =
                    Debt[parent].second ? Debt[parent].first * powered : Data[parent << 1U] + Data[(parent << 1U) | 1U];
        ...
        std::vector<std::pair<value_t, bool>> Debt;
