...
namespace
{
    ...
    {
        test_cases.emplace_back("abcebbe", 3);
        test_cases.emplace_back("abcdabd", 6);
        test_cases.emplace_back("abc", 2);
        test_cases.emplace_back("aaaaac", 5);
        test_cases.emplace_back("abbaca", 3);
        test_cases.emplace_back("abbc", 3);
    ...
    void run_test_case(const test_case& test_case)
    {
        ...
        const auto actual = string_min_appends_to_palindrome(s);
        if (test_case.expected != not_computed)
            Assert::AreEqual(test_case.expected, actual, "string_min_appends_to_palindrome");

        const auto slow = string_min_appends_to_palindrome_slow(s);
        Assert::AreEqual(actual, slow, "string_min_appends_to_palindrome_slow");
    }
}

void Privet::Algorithms::Numbers::Tests::string_min_appends_to_palindrome_tests()
{
