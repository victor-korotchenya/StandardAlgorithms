...
namespace Privet::Algorithms::Trees
{
    ...
    class segment_tree_add_sum final
    {
        int _size_log, _size;
        std::vector<value_t> _ar, _debt;

    public:
        explicit segment_tree_add_sum(int size_log = 18) : _size_log(size_log),
            _size(1 << _size_log), _ar(static_cast<int64_t>(_size) << 1), _debt(_size)
        {
        ...
        void init()
        {
            ...
            for (auto i = _size - 1; i; --i)
                _ar[i] = _ar[static_cast<int64_t>(i) << 1] + _ar[static_cast<int64_t>(i) << 1 | 1];
        }
    ...
    private:
        ...
        void push_down(int left)
        {
            ...
            do
            {
                ...
                if (_debt[p])
                {
                    add_mult(p << 1, _debt[p], k);
                    add_mult(p << 1 | 1, _debt[p], k);
                    _debt[p] = {};
                }
        ...
        void push_up(int left)
        {
            ...
            for (;;)
            {
                ...
                const auto new_value = _debt[left] * mult + _ar[static_cast<int64_t>(left) << 1] + _ar[static_cast<int64_t>(left) << 1 | 1];
                _ar[left] = new_value;
                mult <<= 1;
            }
        }
