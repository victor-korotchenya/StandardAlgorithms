...
class InfInt
    ...
    final
{
    // fix: move "#ifdef" inside the class.
#ifdef INFINT_USE_SHORT_BASE // uses 10^4 (short) as the base
    using ELEM_TYPE = short;
    using PRODUCT_TYPE = int;
    static constexpr ELEM_TYPE BASE = 10000;
    static constexpr ELEM_TYPE UPPER_BOUND = 9999;
    static constexpr ELEM_TYPE DIGIT_COUNT = 4;
    static const int powersOfTen[] = { 1, 10, 100, 1000 };
#else // uses 10^9 (int) as the base
    using ELEM_TYPE = int;
    using PRODUCT_TYPE = long long;
...
public:
  ...
  //InfInt(long l);
    ...
    //Hash.
    // fix
    size_t operator()(const InfInt& b) const;
...
//Hash.
// fix
inline size_t InfInt::operator()(const InfInt& b) const
{
    size_t result = b.pos;
    for (const ELEM_TYPE& value : b.val)
    {
        result = result * size_t(13) + size_t(value);
    }
...
inline const InfInt& InfInt::operator*=(InfInt::ELEM_TYPE rhs)
{
...
inline InfInt InfInt::operator*(InfInt::ELEM_TYPE rhs) const
{//PROFILED_SCOPE
    InfInt result = *this;
    ELEM_TYPE factor = rhs < 0 ? -rhs : rhs;
...
inline size_t InfInt::size() const noexcept
{//PROFILED_SCOPE
...
{//PROFILED_SCOPE
// fix for Linux g++: just comment out.
//
        ...
        for (; i >= 0; --i)
        {
            if (val[i] < 0)
            {
                ...
                { // number on the left is positive
                    ...
                    for (; k > 0; --k)
                    {
                        val[i + static_cast<int64_t>(k)] = UPPER_BOUND;
                    }
                }
            }
        }
        ...
        for (; i >= 0; --i)
        {
            if (val[i] > 0)
            {
                ...
                { // number on the left is negative
                    ...
                    for (; k > 0; --k)
                    {
                        val[static_cast<int64_t>(i) + k] = -UPPER_BOUND;
                    }
                }
            }
        }
...
{//PROFILED_SCOPE
    ...
    if (i > -DIGIT_COUNT)
    {
        std::string ss = s.substr(0, static_cast<int64_t>(i) + DIGIT_COUNT);
        if (ss.size() == 1 && ss[0] == '-')
        {
            pos = false;
        }
...
inline InfInt::ELEM_TYPE InfInt::dInR(const InfInt& R, const InfInt& D)
{//PROFILED_SCOPE
...
inline void InfInt::multiplyByDigit(
    InfInt::ELEM_TYPE factor, std::vector< InfInt::ELEM_TYPE >& val)
{//PROFILED_SCOPE
    ELEM_TYPE carry = 0;
    for (size_t i = 0; i < val.size(); ++i)
    {
        ...
        if (pval >= BASE || pval <= -BASE)
        {
            ...
            carry = (ELEM_TYPE)(pval / BASE);
            pval -= carry * static_cast<int64_t>(BASE);
        }
        else
        {
            carry = 0;
...
{//PROFILED_SCOPE
    ...
    for (int i = (int)n.val.size() - 1; i >= 0; --i)
    {
        ...
        {
            s << std::setfill('0') << std::setw(InfInt::DIGIT_COUNT) << n.val[i];
        }
    }
    return s;
}
